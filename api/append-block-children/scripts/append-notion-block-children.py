...
def main() -> None:
    if len(sys.argv) < 3:
        raise SystemExit("usage: append-notion-block-children.py <block_id> <json_data>")
    block_id = sys.argv[1]
    raw_data = sys.argv[2]
    data = resolve_data(raw_data)
    raw = run_curl("patch", f"/blocks/{block_id}/children", data)
